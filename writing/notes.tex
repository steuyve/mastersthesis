...
and
\begin{align*}
L = &\sum\limits_{\substack{2\leq p_2\leq m+1 \\ 0\leq q_1 \leq p_2-2 \\ 0 \leq q_2 \leq m+1-p_2}}\omega_2(L_{1,p_2,q_1,q_2}^{m+1}) + \sum\limits_{\substack{2 \leq p_1 \leq m-1 \\ 0 \leq q_1 \leq m-1-p_1}}\omega_2(L_{p_1,m,q_1,1}^{m+1}) \\
...
Consider a disjoint $(m+1)$-collection in the second case, that is, we fix $2\leq p_1\leq m-1$,$p_2=m$,$0\leq q_1\leq m-1-p_1$, $q_2 = 1$. Then we have
\begin{equation*}
...
\begin{equation}
\begin{aligned}\label{eqn:simplifiedlast}
...
So substituting (\ref{eqn:innersumsimp2}) into (\ref{eqn:simplifiedlast}) above we have
...
Expanding, the second sum in (\ref{eqn:simplifiedlast2}) simplifies to:
\begin{equation}\label{eqn:secondsum}
\begin{aligned}
...
\end{aligned}
\end{equation}
where the second to last equality is given by Wolfram Alpha.

The first sum in (\ref{eqn:simplifiedlast2}) simplifies to:
\begin{equation}\label{eqn:firstsum}
\begin{aligned}
...
So putting (\ref{eqn:firstsum}) and (\ref{eqn:secondsum}) together we have
\begin{equation}\label{eqn:simplifiedlast3}
\begin{aligned}
V_1\sum\limits_{\substack{1\leq p \leq m -1 \\ 0 \leq q \leq m-1-p}}\frac{1}{2p+1}\prod\limits_{k=p+1}^{p+q}\left(\frac{2k}{2k+1}\right) = 2\binom{m-\frac{1}{2}}{m}^{-1}\left(m+1-\frac{2^{2m}}{(m+1)C_m}\right).
\end{aligned}
\end{equation}
