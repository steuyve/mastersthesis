...
The convex magnitude conjecture predicts that (\ref{eqn:goal}) is given by
\begin{equation*}
\frac{1}{2\omega_2}V_2\left(B_2^d\right) = \frac{1}{2\pi}V_2\left(B_2^d\right).
\end{equation*}
By Theorem 9.2.4 of \cite{klain_geometric_1997}, we have that
\begin{equation*}
V_2\left(B_2^d\right) = \binom{d}{2}\frac{\omega_d}{\omega_{d-2}} = \binom{d}{2}\pi\frac{\Gamma\left(\frac{d}{2}\right)}{\Gamma\left(\frac{d}{2}+1\right)} = \binom{d}{2}\frac{2\pi}{d}.
\end{equation*}
So the convex magnitude conjecture predicts that
\begin{equation}\label{eqn:secondorderpred}
\frac{d^2}{dt^2}\text{Mag}(tB_2^d)\big\vert_{t=0} = \frac{1}{2\pi}V_2\left(B_2^d\right) = \frac{1}{d}\binom{d}{2} = \frac{1}{2}(d-1).
\end{equation}

To compute the value of (\ref{eqn:goal}), we will follow the same general approach that Meckes used to prove Theorem \ref{theo:firstorder} in \cite{meckes_magnitude_2019}. An outline is presented below:
\begin{enumerate}[label=\arabic*.]
...
\begin{aligned}
\sigma = &\sum\limits_{\substack{2\leq p_2\leq m+1 \\ 0\leq q_1 \leq p_2-2 \\ 0 \leq q_2 \leq m+1-p_2}}\omega_2(\sigma_{1,p_2,q_1,q_2}^{m+1}) + \sum\limits_{\substack{2 \leq p_1 \leq m-1 \\ 0 \leq q_1 \leq m-1-p_1}}\omega_2(\sigma_{p_1,m,q_1,1}^{m+1}) \\
...
\begin{aligned}
L = &\sum\limits_{\substack{2\leq p_2\leq m+1 \\ 0\leq q_1 \leq p_2-2 \\ 0 \leq q_2 \leq m+1-p_2}}\omega_2(L_{1,p_2,q_1,q_2}^{m+1}) + \sum\limits_{\substack{2 \leq p_1 \leq m-1 \\ 0 \leq q_1 \leq m-1-p_1}}\omega_2(L_{p_1,m,q_1,m}^{m+1}) \\
...
Consider a disjoint $(m+1)$-collection in the second case, that is, we fix $2\leq p_1\leq m-1$,$p_2=m$,$0\leq q_1\leq m-1-p_1$, $q_2 = 1$. Then each summand is given by
\begin{equation*}
...
\begin{figure}[h!]
\centerline{\includegraphics[width=12cm]{secs.eps}}\caption{\label{fig:secondsplot} Second order terms computed from (\ref{eqn:partialres}) in Proposition \ref{prop:partialres} above for odd dimensions from 3 to 41 alongside the values as predicted by the erstwhile convex magnitude conjecture (\ref{eqn:secondorderpred}).}
\end{figure}
