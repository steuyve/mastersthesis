...
We first define the magnitude of a matrix. Let $M \in M_n(\mathbb{R})$. The vector $w \in \mathbb{R}^n$ is called a \textbf{weighting} if $Mw = 1_n$, the vector of all $1$'s. Similarly, $v \in \mathbb{R}^n$ is called a \textbf{coweighting} if $v^\star M = 1_n^\star$.
\begin{lem}
...
\end{equation*}
\end{proof}
The lemma ensures that if a matrix $M$ admits both a weighting and a coweighting, then the sum of all the entries of the (co)weighting is independent of the (co)weighting chosen. This means we can make the following definition without ambiguity:
\begin{defn}
Let $M \in M_n(\mathbb{R})$. If $M$ has a weighting $w$ and a coweighting $v$, then its \textbf{magnitude} is defined and is given by
\begin{equation*}
...
\begin{equation*}
Mx = 1_n \qquad x^\star M = 1_n^\star
\end{equation*}
...
\begin{defn}
Let $A$ be a finite metric space with $n$ points and distance function $d$. Then the \textbf{similarity matrix} $Z_A$ is the $n\times n$ real matrix given by $[Z_A]_{ij} = e^{-d(i,j)}$ where $i,j \in A$. The \textbf{magnitude} of $A$, also denoted $\vert A \vert$ is the magnitude of its similarity matrix. 
\end{defn}
...
\begin{proof}
Let $\#A = n$. For an $n \times n$ invertible matrix $M$, by Proposition \ref{prop:inv}, the unique weighting $w$ on $M$ is given by
\begin{equation}
...
\end{equation}
where $M\text{adj}(M) = (\det{M})I_n$. Importantly, the adjugate is a smooth function of $M$ (see section 0.8 of \cite{horn_matrix_2013}).
...
So for any finite metric space $A$, the value of the magnitude function approaches the cardinality of $A$ as we blow up distances between points. However, along with the fact that the magnitude may not be defined everywhere, other ``nice'' properties that we might expect from a function, such as monotonicity, are not guaranteed, as the following example shows.
...
Given the example above, it is natural to ask whether there are classes of finite metric spaces for which the magnitude function is more well-behaved.

\begin{defn}
A finite metric space $A$ is \textbf{positive definite} if its similarity matrix $Z_A$ is a positive definite matrix.
\end{defn}

Since positive definite matrices are invertible, any positive definite metric space has magnitude. Likewise, since prinicipal submatrices of positive definite matrices are positive definite, subspaces of positive definite metric spaces are positive definite and hence also have magnitude.

The main attractiveness of positive definite metric spaces is that we have a more convenient explicit formula for the magnitude.
