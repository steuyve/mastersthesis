...
\begin{equation*}
\mathbf{Z}_{tK_{3,2}} = \begin{bmatrix} 1 & e^{-2t} & e^{-2t} & e^{-t} & e^{-t} \\
e^{-2t} & 1 & e^{-2t} & e^{-t} & e^{-t} \\
e^{-2t} & e^{-2t} & 1 & e^{-t} & e^{-t} \\
e^{-t} & e^{-t} & e^{-t} & 1 & e^{-2t} \\
e^{-t} & e^{-t} & e^{-t} & e^{-2t} & 1
...
\begin{defn}
A finite metric space $A$ is \textbf{positive definite} if its similarity matrix $\mathbf{Z}_A$ is a positive definite matrix.
\end{defn}
...
\begin{equation*}
\vert A \vert = \sup\limits_{v \neq 0}\frac{\left(\sum_{a\in A}v_a\right)^2}{v^\star\mathbf{Z}_Av}
\end{equation*}
where $v \in \mathbb{R}^{\#A}$. $v$ attains the supremum if and only if it is a nonzero scalar multiple of the unique weighting on $A$.
\end{theo}

\begin{proof}
Since $\mathbf{Z}_A$, we have the Cauchy-Schwarz inequality:
\begin{equation*}
(v^\star\mathbf{Z}_A w)^2 \leq (v^\star\mathbf{Z}_Av)(w^\star\mathbf{Z}_Aw)
\end{equation*}
with equality if and only if $v,w$ are scalar multiples of each other. We take $w$ to be the unique weighting on $\mathbf{Z}_A$. Then we have
\begin{equation*}
\vert A \vert = \sum\limits_{a\in A}w_a = w^\star\mathbf{Z}_Aw \geq \frac{(v^\star\mathbf{Z}_Aw)^2}{v^\star\mathbf{Z}_Av} = \frac{\left(\sum_{a\in A}v_a\right)^2}{v^\star\mathbf{Z}_Av}
\end{equation*}
for all $v$. So $\vert A \vert$ is greater than or equal to the supremum. But taking $w$ to be the unique weighting gives
\begin{equation*}
\frac{\left(\sum_{a\in A}w_a\right)^2}{w^\star\mathbf{Z}_Aw} = \frac{(w^\star\mathbf{Z}_Aw)^2}{w^\star\mathbf{Z}_Aw} = w^\star\mathbf{Z}_Aw = \sum\limits_{a\in A}w_a = \vert A \vert
\end{equation*}
so we have equality.
\end{proof}
...
\begin{equation*}
\vert B \vert = \sup\limits_{v\neq0}\frac{(\sum_{b\in B} v_b)^2}{v^\star\mathbf{Z}_Bv} \leq \sup\limits_{v\neq0}\frac{(\sum_{a\in A}v_a)^2}{v^\star\mathbf{Z}_Av} = \vert A\vert
\end{equation*}
